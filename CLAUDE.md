...
- Well-specified, chunked tasks needing a fast second opinion.
- Bounded answers, research, review, or test-plan drafting.
- Scoped patch proposal generation for explicitly identified files.
...
- `scripts/gemini_delegate.py` — single-mode delegate runner (`patch`, `review`, `tests`, `alt`, `research`, `answer`)
...
```bash
# Single answer pass
echo "TASK: ..." | scripts/gemini_delegate.py --mode answer

# Single research pass
echo "TASK: ..." | scripts/gemini_delegate.py --mode research
...
# Parallel fan-out
cat payload.txt | scripts/gemini_fanout.py --jobs answer research review tests alt --concurrency 3
```
...
5. Defaults to `--model gemini-3-flash-preview` and auto-falls back to `--fallback-model auto` on capacity errors.
